...
class Ybc_blog extends Module
{
    ...
    public function __construct()
	{
        ...
        $this->configs = array(
            ...
            'YBC_BLOG_DATE_FORMAT' => array(
                ...
                'default' => 'j.m.Y',
                'width' => 200,
                'desc' => $this->l('Default: j.m.Y, Check more at http://php.net/manual/en/function.date.php')             
            ), 
            //SEO
            ...
            'YBC_BLOG_ENABLE_POST_SLIDESHOW' => array(
                'label' => $this->l('Enable post image slideshow'),
                'type' => 'switch',
                'default' => 1,                
                'tab' => 'blog',
            ), 
            ...
            'YBC_BLOG_ALLOW_COMMENT_LOGIN' => array(
                'label' => $this->l('Allow comment when not logged'),
                'type' => 'switch',
                'default' => 1,                
                'tab' => 'comment',
            ),
        ...
        if(!$this->is17 && isset($this->configs['YBC_BLOG_SIDEBAR_POSITION']))
            unset($this->configs['YBC_BLOG_SIDEBAR_POSITION']);       
    ...
    public function updateCategories($categories, $id_post)
    {
        ...
        {            
            foreach($categories as $cat)
            {
                ...
                {
                    $req = "INSERT INTO "._DB_PREFIX_."ybc_blog_post_category(id_post, id_category) VALUES(".(int)$id_post.", ".(int)$cat.")";
                    Db::getInstance()->execute($req);   
                }                
            }
        }
    ...
    {
        $req = "UPDATE "._DB_PREFIX_."ybc_blog_$tbl SET `$field`=".(int)$status." WHERE id_$tbl=".(int)$id;
    ...
    public function getPostsByIdCategory($id_category, $id_lang = false, $enabled = false)
    {
        ...
        $req = "SELECT p.*, pl.title, pl.description ,pl.short_description , pl.meta_keywords, pl.meta_description
                ...
                JOIN "._DB_PREFIX_."ybc_blog_post_category pc ON p.id_post = pc.id_post AND pc.id_category=".(int)$id_category."
     ...
     */
     ...
     private function _postConfig()
     {
        ...
        if(Tools::isSubmit('saveConfig'))
        {
            ...
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS')))
                $errors[] = $this->l('Blog alias is not valid');
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS_POST')))
                $errors[] = $this->l('Single post page alias');
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS_CATEGORY')))
                $errors[] = $this->l('Category page alias is not valid');
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS_GALLERY')))
                $errors[] = $this->l('Gallery page alias is not valid');
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS_LATEST')))
                $errors[] = $this->l('Latest posts page alias is not valid');
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS_SEARCH')))
                $errors[] = $this->l('Search page alias is not valid');
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS_AUTHOR')))
                $errors[] = $this->l('Author page alias is not valid');
            if(!Validate::isLinkRewrite(Tools::getValue('YBC_BLOG_ALIAS_TAG')))
                $errors[] = $this->l('Tag page alias is not valid');
            if(!$errors)
            {
                $aliasArg = array('YBC_BLOG_ALIAS','YBC_BLOG_ALIAS_POST','YBC_BLOG_ALIAS_CATEGORY','YBC_BLOG_ALIAS_GALLERY','YBC_BLOG_ALIAS_LATEST','YBC_BLOG_ALIAS_SEARCH','YBC_BLOG_ALIAS_AUTHOR','YBC_BLOG_ALIAS_TAG');
                $alias = array();
                foreach($aliasArg as $aliaKey)
                {
                    $postedAlias = trim(Tools::getValue($aliaKey));
                    
                    if($postedAlias && in_array($postedAlias,$alias))
                    {
                        $errors[] = $this->l('Alias need to be unique');
                        break;                        
                    }
                    elseif($postedAlias){
                        $alias[] = $postedAlias;
            ...
            if((int)Tools::getValue('YBC_BLOG_POST_EXCERPT_LENGTH') < 0)
                $errors[] = $this->l('Post excerpt length cannot be smaller than 0');
            if((int)Tools::getValue('YBC_BLOG_GALLERY_PER_PAGE') <= 0)
     ...
     public function getLink($controller = 'blog', $params = array())
     {
        ...
        {            
            ...
            {                
               $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_GALLERY')) ? $subAlias : 'gallery').($page ? '/'.rtrim($page,'/') : '');
               return $url;
            }
            elseif($controller=='blog')
            {
                ...
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_POST')) ? $subAlias : 'post').'/'.(int)$params['id_post'].'-'.$postAlias.$subfix;
                }
                elseif(isset($params['id_category']) && $categoryAlias = $this->getCategoryAlias((int)$params['id_category']))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_CATEGORY')) ? $subAlias : 'category').'/'.$page.(int)$params['id_category'].'-'.$categoryAlias.$subfix;
                }
                elseif(isset($params['id_author']) && $authorAlias = (isset($params['alias']) ? $params['alias'] : 'author'))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_AUTHOR')) ? $subAlias : 'author').'/'.$page.(int)$params['id_author'].'-'.$authorAlias;
                }
                elseif(isset($params['tag']))
                {
                    $url .= $page.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_TAG')) ? $subAlias : 'tag').'/'.(string)$params['tag'];
                }
                elseif(isset($params['search']))
                {
                    $url .= $page.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_SEARCH')) ? $subAlias : 'search').'/'.(string)$params['search'];
                }
                elseif(isset($params['latest']))
                {
                    $url .= (($subAlias = Configuration::get('YBC_BLOG_ALIAS_LATEST')) ? $subAlias : 'latest').($page ? '/'.rtrim($page,'/') : '');
                }
                else
                {
                    if($page)
     ...
     {          
        return Db::getInstance()->getRow("SELECT bc.*, c.firstname, c.lastname, c.email, e.firstname as efirstname, e.lastname as elastname,pl.title as post_title
                FROM "._DB_PREFIX_."ybc_blog_comment bc
                LEFT JOIN "._DB_PREFIX_."customer c ON c.id_customer = bc.id_user
                LEFT JOIN "._DB_PREFIX_."employee e ON e.id_employee = bc.replied_by
       ...
       public function renderCommentsForm()
       {
    			...
    			'form' => array(
    				...
    				'input' => array(					
    					array(
                            ...
                            'required' => true,
                            'hint' => $this->l('Invalid characters:').' &lt;&gt;;=#{}',
                            'desc' => ($id_comment = (int)Tools::getValue('id_comment')) && ($comment = $this->getCommentById($id_comment)) ? $this->displayCommentInfo($comment,$this->context->link->getAdminLink('AdminCustomers').'&id_customer='.(int)$comment['id_user'].'&updatecustomer',$this->getLink('blog',array('id_post' => (int)$comment['id_post']))) : '',	                    
    					), 
                        array(
        					'type' => 'select',
        					'label' => $this->l('Rating'),
        ...
        public function hookModuleRoutes($params) {
            ...
            $routes = array(
                ...
                'ybcblogpost' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_POST')) ? $subAlias : 'post').'/{id_post}-{url_alias}'.$subfix,
                    'keywords' => array(
                ...
                'categoryblogpost' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_CATEGORY')) ? $subAlias : 'category').'/{id_category}-{url_alias}'.$subfix,
                    'keywords' => array(
                ...
                'categoryblogpostpage' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_CATEGORY')) ? $subAlias : 'category').'/{page}/{id_category}-{url_alias}'.$subfix,
                    'keywords' => array(
                        'id_category' =>    array('regexp' => '[0-9]+', 'param' => 'id_category'),
                        'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                        'url_alias'       =>   array('regexp' => '[_a-zA-Z0-9-]+','param' => 'url_alias'),
                ...
                'authorblogpost' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_AUTHOR')) ? $subAlias : 'author').'/{id_author}-{author_name}',
                    'keywords' => array(
                ...
                'authorblogpostpage' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_AUTHOR')) ? $subAlias : 'author').'/{page}/{id_author}-{author_name}',
                    'keywords' => array(
                        'id_author' =>    array('regexp' => '[0-9]+', 'param' => 'id_author'),
                        'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                        'author_name'       =>   array('regexp' => '(.)+','param' => 'author_name'),
                ...
                'categoryblogtag' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_TAG')) ? $subAlias : 'tag').'/{tag}',
                    'keywords' => array(
                ...
                'categoryblogtagpage' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/{page}/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_TAG')) ? $subAlias : 'tag').'/{tag}',
                    'keywords' => array(
                        'tag'       =>   array('regexp' => '.+','param' => 'tag'),
                        'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                    ),
                ...
                'categorybloglatest' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_LATEST')) ? $subAlias : 'latest'),
                    'keywords' => array(),
                    'params' => array(
                        'fc' => 'module',
                        'module' => 'ybc_blog',
                ...
                'categorybloglatestpage' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_LATEST')) ? $subAlias : 'latest').'/{page}',
                    'keywords' => array(                       
                        'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                    ),
                    'params' => array(
                ...
                'categoryblogsearch' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_SEARCH')) ? $subAlias : 'search').'/{search}',
                    'keywords' => array(
                ...
                'categoryblogsearchpage' => array(
                    'controller' => 'blog',
                    'rule' => $blogAlias.'/{page}/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_SEARCH')) ? $subAlias : 'search').'/{search}',
                    'keywords' => array(
                        'search'       =>   array('regexp' => '.+','param' => 'search'),
                        'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                    ),
                ...
                'categorybloggallerypage' => array(
                    'controller' => 'gallery',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_GALLERY')) ? $subAlias : 'gallery').'/{page}',
                    'keywords' => array(
                        'page' =>    array('regexp' => '[0-9]+', 'param' => 'page'),
                    ),
                    'params' => array(
                ...
                'categorybloggallery' => array(
                    'controller' => 'gallery',
                    'rule' => $blogAlias.'/'.(($subAlias = Configuration::get('YBC_BLOG_ALIAS_GALLERY')) ? $subAlias : 'gallery'),
                    'keywords' => array(),
                    'params' => array(
                        'fc' => 'module',
                        'module' => 'ybc_blog',
            ...
            if($tags && is_array($tags))
            {
                ...
                if($tagElements)
                {
                    ...
                    $sql = "SELECT pl.title, pl.short_description, p.*
                        ...
                        LEFT JOIN "._DB_PREFIX_."ybc_blog_post_lang pl ON pl.id_post = t.id_post AND pl.id_lang = ".(int)$id_lang."
